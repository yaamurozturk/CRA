...
def replaceMultiple(mainString, toBeReplaces):
    ...
    for elem in toBeReplaces:
        ...
        item3 = '(' + elem + "."
        item4 = '(' + elem + " ."
        if item1 in mainString:
            mainString = mainString.replace(item1, ' ' + elem)
        if mainString.startswith(item1[1:]) is True:
            mainString = mainString.replace(item1[1:], elem)
        if item2 in mainString:
            mainString = mainString.replace(item2, ' ' + elem)
        if mainString.startswith(item2[1:]) is True:
            mainString = mainString.replace(item2[1:], elem)
        if item3 in mainString:
            mainString = mainString.replace(item3, '(' + elem)
        if item4 in mainString:
            mainString = mainString.replace(item4, '(' + elem)

    return mainString


def find_abbr(some_str):
    pattern = r'^[A-Z]\s?\.(?:\s?[A-Z]\s?\.)*[\W\s]|^[A-Z]\s?\.(?:\s?[A-Z]\s?\.)*$|[\s(](?:[\s]?[A-Z]\s?\.)+[\W\s]|[\s(](?:[\s]?[A-Z]\s?\.)+$'  # find abbr 1 uppercase letter, like: A.
    find = re.findall(pattern, some_str)
    for abbr in find:
        if abbr[-1] == '.':
            some_str = some_str.replace(abbr, abbr[:-1] + ' ')
        elif abbr[-2] == '.':
            some_str = some_str.replace(abbr, abbr[:-2] + abbr[-1])

    pattern1 = r'(?:\s[a-z]\.)+(?:\s|[^a-zA-Z]){,4}[a-z]{2,}'  # find case like: a. aaaa is
    find1 = re.findall(pattern1, some_str)
    for abbr1 in find1:
        abbr1_replace_as = abbr1.replace('.', '')
        some_str = some_str.replace(abbr1, abbr1_replace_as)

    pattern1_1 = r'(?:\s|[^a-zA-Z\.])(?:[a-z]\.)+(?:\s|[^a-zA-Z\.])'  # find case like: r.p.m.
    find1_1 = re.findall(pattern1_1, some_str)
    for abbr1_1 in find1_1:
        abbr1_1_replace_as = abbr1_1.replace('.', '')
        some_str = some_str.replace(abbr1_1, abbr1_1_replace_as)

    pattern2_0 = r'(?:(?:\s|[-–−])[A-Z][a-z]{2,}\.)+(?:\s|[^a-zA-Z])'  # find case like Aaa. Bbb. or -Aaa. Bbb. or Aaa.
    find2_0 = re.findall(pattern2_0, some_str)
    for abbr2_0 in find2_0:
        if abbr2_0 + '|' not in some_str:
            abbr2_0_replace_as = abbr2_0.replace('.', '')
            some_str = some_str.replace(abbr2_0, abbr2_0_replace_as)

    pattern2_1 = r'(?:\s[a-z]{3,}\.){2,}(?:\s|[^a-zA-Z])'  # find case like aaa. bbbbb.
    find2_1 = re.findall(pattern2_1, some_str)
    for abbr2_1 in find2_1:
        abbr2_1_replace_as = abbr2_1.replace('.', '')
        some_str = some_str.replace(abbr2_1, abbr2_1_replace_as)

    pattern2_2 = r'(?:[^A-Za-z][A-Z][a-z]{2,}\.)+\s{1,2}[^A-Za-z]'  # find case like (Aaa. :
    find2_2 = re.findall(pattern2_2, some_str)
    for abbr2_2 in find2_2:
        abbr2_2_replace_as = abbr2_2.replace('.', '')
        some_str = some_str.replace(abbr2_2, abbr2_2_replace_as)

    pattern2_3 = r'[\da-z]\.\s?\[\s?[a-zA-Z\d]+\s?\]\s?'  # find case like: 2011. [MB560650]
    find2_3 = re.findall(pattern2_3, some_str)
    for abbr2_3 in find2_3:
        abbr2_3_replace_as = abbr2_3.replace('.', '')
        some_str = some_str.replace(abbr2_3, abbr2_3_replace_as)

    pattern4 = r'\s[A-Z][a-z]{2,}\s{1,2}[A-Z][a-z]{2,}\.[:;,]'  # find case like: Aaa Aaa.:
    find4 = re.findall(pattern4, some_str)
    for abbr4 in find4:
        abbr4_replaced_as = abbr4.replace('.', '')
        some_str = some_str.replace(abbr4, abbr4_replaced_as + ' ')

    pattern3_1 = r'(?:\s\d{2,}\.\s{1,3}\d{2,})'  # find case like: 12. 123
    find3_1 = re.findall(pattern3_1, some_str)
    for abbr3_1 in find3_1:
        replaced_as = abbr3_1.replace('. ', ' ')
        some_str = some_str.replace(abbr3_1, replaced_as)

    pattern5 = r'[\w\s][^a-zA-Z\d][A-Z]\.\s'  # find cas like: =A.
    find5 = re.findall(pattern5, some_str)
    for abbr5 in find5:
        some_str = some_str.replace(abbr5, abbr5[:-2] + ' ')

    return some_str


def preprocess_text(a_long_string):
    replaceitem = ['Abbr', 'Acad', 'Ae', 'Alt', 'Am', 'An', 'Assn', 'At. no', 'At. no', 'At. wt', 'At. wt', 'Aug',
                   'Ave', 'Awol', 'Btu', 'Ca', 'Cal', 'Capt', 'Cca', 'Cent', 'Cf', 'Cir', 'Cit', 'Cm', 'Co', 'Col',
                   'Comdr', 'Corp', 'Cpl', 'Cu', 'Cx', 'Dec', 'Dept', 'Dist', 'Div', 'Dr', 'Ed', 'Eq', 'Est', 'Et al',
                   'Etal', 'Etc', 'Feb', 'Fig', 'Figs', 'Fl', 'Fl oz', 'Fm', 'Ft', 'Gal', 'Gen', 'Gmt', 'Gnp', 'Gop',
                   'Gov', 'Grad', 'Hon', 'Hr', 'Ibid', 'Id', 'In', 'Inc', 'Inf', 'Inst', 'Ira', 'Irs', 'Jan', 'Jr',
                   'Kg', 'Km', 'Lat', 'Lb', 'Lib', 'Loc', 'Long', 'Lt', 'Ltd', 'Lu', 'Ma', 'Mg', 'Mi', 'Min', 'Mm',
                   'Mph', 'Mr', 'Mrs', 'Msgr', 'Mt', 'Mts', 'Mus', 'Naacp', 'Nasa', 'Nato', 'Ne', 'NIH', 'No', 'Nov',
                   'NSF', 'Oas', 'Oct', 'Op', 'Oz', 'Pl', 'Pop', 'Pseud', 'Pt', 'Pub', 'Qt', 'Ref', 'Refs', 'Rev',
                   'Rpm', 'Rr', 'Sc', 'Seato', 'Sec', 'Sept', 'Ser', 'Sgt', 'Sic', 'Sp', 'Spp', 'Sq', 'Sr', 'Ssr', 'St',
                   'Sup', 'Suppl', 'Unicef', 'Uninc', 'Univ', 'Usa', 'Usaf', 'Uscg', 'Usmc', 'Usn', 'Ussr', 'Vfw',
                   'Vista', 'Viz', 'Vol', 'Vs', 'Who', 'Wt', 'Yd', 'Ymca', 'Ywca', 'abbr', 'acad', 'ae', 'al', 'alt',
                   'am', 'assn', 'at. no', 'at. no', 'at. wt', 'at. wt', 'aug', 'ave', 'awol', 'btu', 'ca', 'cal',
                   'capt', 'cca', 'cent', 'cf', 'cir', 'cit', 'cm', 'co', 'col', 'comdr', 'corp', 'cpl', 'cu', 'cx',
                   'dec', 'dept', 'dist', 'div', 'dr', 'ed', 'eg', 'eq', 'est', 'et al', 'et at', 'etal', 'etc', 'feb',
                   'fig', 'figs', 'fl', 'fl oz', 'fm', 'ft', 'gal', 'gen', 'gmt', 'gnp', 'gop', 'gov', 'grad', 'hon',
                   'hr', 'i.vag', 'ibid', 'id', 'ie', 'in', 'inc', 'inf', 'inst', 'ira', 'irs', 'jan', 'jr', 'kg', 'km',
                   'lat', 'lb', 'lib', 'loc', 'long', 'lt', 'ltd', 'mg', 'mi', 'min', 'mm', 'mph', 'mr', 'mrs',
                   'msgr', 'mt', 'mts', 'mus', 'naacp', 'nasa', 'nato', 'ne', 'no', 'nov', 'oas', 'obs', 'oct', 'op',
                   'oz', 'pl', 'pop', 'pp', 'pseud', 'pt', 'pub', 'qt', 'ref', 'refs', 'resp', 'rev', 'rpm', 'rr',
                   'sc', 'seato', 'sec', 'sept', 'ser', 'sgt', 'sic', 'sp', 'spp', 'sq', 'sr', 'ssr', 'st', 'subsp',
                   'sup', 'suppl', 'unicef', 'uninc', 'univ', 'usa', 'usaf', 'uscg', 'usmc', 'usn', 'ussr', 'vfw',
                   'vista', 'viz', 'vol', 'vs', 'who', 'wt', 'yd', 'ymca', 'ywca'] + ['FIG', 'Par', 'var', 'Fr',
                                                                                      '— Herb', 'unpubl', 'equiv']

    a_long_string = a_long_string.replace('\t', ' ').replace('\n', ' ')
    a_long_string = ' '.join(a_long_string.split())
    a_long_string = a_long_string.replace(' . ', '. ')
    a_long_string = find_abbr(a_long_string)
    a_long_string = a_long_string.replace(' .', '.')
    a_long_string = a_long_string.replace('i.e.', 'i.e').replace('e.g.', 'e.g').replace(' al.', ' al').replace('Fig.', 'Fig').replace(' fig.', ' fig')
    a_long_string = replaceMultiple(a_long_string, replaceitem)
    a_long_string = ' '.join(a_long_string.split())

    return a_long_string


def remove_special_characters(some_text, remove_digits=False):
    pattern = r'[^a-zA-Z0-9\s]' if not remove_digits else r'[^a-zA-Z\s]'
    some_text = re.sub(pattern, ' ', some_text)
...
def split_sentences(text):
    ...
    cleaned_text = re.sub(
        r'\[(?:[^\[\]]*?[.;][^\[\]]*?)\]',
        lambda m: m.group(0).replace('.', '<DOT>').replace(';', '<SEMI>'),
        cleaned_text
    )
    split_regex = r'(?<=[.!?])\s+(?=[A-Z0-9])'
    sentences = re.split(split_regex, cleaned_text)

...
def find_section_title(element):
    """Traverse up the XML tree to find the nearest top-level section title."""
    while element is not None:
        parent = element.getparent()
        if parent is not None and parent.tag == "sec":
            # Check if the parent <sec> has a <title> child and it's a top-level <sec>
            # Ensure we're not in a nested <sec>
            grandparent = parent.getparent()
            if grandparent is not None and grandparent.tag != "sec":
...
def extract_pmc_citations(xml_file):
    """Extract citations from the first <article> tag of a given PMC XML file and stop if a second <article> tag appears."""
    try:
        ...
        # Locate the first article and stop if a second appears
        for article in root.iter("article"):
            article_count += 1
            if article_count == 1:
                first_article = article
                #print(ET.tostring(first_article, encoding='utf-8').decode('utf-8'))

            elif article_count > 1:
                #print("Second <article> tag found. Stopping extraction.")
                break  # Stop processing further articles

        
        # Get DOI of citing article
        ...
        for ref in first_article.findall(".//ref"):
            if ref.find(".//ref-list") is not None:
                break  
            ref_id = ref.get("id")
        ...
        for paragraph in first_article.findall(".//p"):
            text = " ".join(paragraph.itertext()).strip() if paragraph.text else ""
            sentences = split_sentences(text)
            
            citation_matches = paragraph.findall(".//xref")
            position = find_section_title(paragraph)
                ...
                if citation_id in ref_dict:
                    sentence_index = next(
                        (i for i, s in enumerate(sentences) 
                         if citation.text and re.search(rf'\b{re.escape(citation.text.strip())}\b', s)),
                        None
                    )
        ...
        df = pd.DataFrame(data, columns=["Citing DOI", "Citation ID", "CC paragraph", "CC prev_next", "Position", "Cited title", "DOI"])
        #print(df)
...
print(f"Completed fetching abstracts for {len(final_df)} in {round(time.time() - start_time, 2)} seconds.")
